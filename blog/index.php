...
<?php foreach ($posts_recent as $post_recent): ?>
		        ...
		        <div class="card mb-4">
		          <img class="card-img-top img-max" src="<?php echo './static/images/' . $post_recent['image']; ?>" alt="">
		          <div class="card-body">
		            <h2 class="card-title"><?php echo $post_recent['title'] ?></h2>

								<?php $postbody = html_entity_decode($post_recent['body']);?>

		            <p class="card-text"><?php echo echo_limit($postbody, 300); ?></p>
		            <a href="single_post.php?post-slug=<?php echo $post_recent['slug']; ?>" class="btn btn-primary">Read More &rarr;</a>
		          </div>
		          <div class="card-footer text-muted">
		           <?php echo date("F j, Y ", strtotime($post_recent["created_at"])); ?>
		          </div>
		        </div>
<?php endforeach ?>
</div>
