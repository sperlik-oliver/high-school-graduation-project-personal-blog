<?php
  require_once(ROOT_PATH . '/admin/includes/admin_functions.php');
// Post variables
$post_id = 0;
$isEditingPost = false;
$published = 0;
...
function createPost($request_values)
	{
		...
		if (count($errors) == 0) {
			...
			$query = "INSERT INTO posts (user_id, title, slug, views , image, body, published, created_at, updated_at,visits,unique_visits) VALUES($user_id, '$title', '$post_slug',0, '$featured_image', '$body', $published, now(), now(),0,0)";
	...
	* * * * * * * * * * * * * * * * * * * * * */
	...
	{
		global $conn, $title, $post_slug,$f_image,$body, $published, $isEditingPost, $post_id, $post_topic;
		...
		$post_topic = GetPostTopic($post['id'])['name'];
	}
